...
export function StepGPA({ avgGpa, prevSemesterGpa, onChange, onNext, onPrev }: StepGPAProps) {
  // Local state for string input to allow typing decimals comfortably
  ...
  return (
    <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="space-y-2">
        <h2 className="text-2xl font-bold text-foreground">학점을 입력해주세요</h2>
        <p className="text-muted-foreground">4.5 만점 기준으로 입력해주세요.</p>
      </div>

      <div className="space-y-6">
        <div className="space-y-2">
          <label className="text-base font-semibold text-foreground">전체 평균 평점 (총 평점)</label>
          <Input 
            type="number" 
            placeholder="예: 3.85" 
        ...
        <div className="space-y-2">
          <label className="text-base font-semibold text-foreground">직전 학기 평점</label>
          <Input 
            type="number" 
            placeholder="예: 4.0" 
            className="h-12 text-lg"
